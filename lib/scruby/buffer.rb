  ...
  class Buffer
    ...
    class << self
      def allocate(server, frames: -1, channels: 1, &message)
        new(server, frames: frames, channels: channels).allocate &message
      end
      ...
      def read_channel(server, path, start: 0, frames: -1, channels: [], &message)
        new(server, frames: frames, channels: channels).allocate_read_channel expand_path(path), start, frames, channels, &message
      end

      def alloc_consecutive(buffers, server, frames: -1, channels: 1, &message)
        buffers = Array.new(buffers) { new server, frames: frames, channels: channels }
