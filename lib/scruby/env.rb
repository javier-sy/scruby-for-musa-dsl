module Scruby
  class Env
    ...
    def initialize(levels, times, curve: :lin, release_node: nil, loop_node: nil)
      # times should be one less than levels size

      @levels = levels
      @times = times

      @curves = curve.to_array
    ...
    class << self
      ...
      def sine(dur: 1, level: 1)
        dur *= 0.5
        new [0, level, 0], [dur, dur], curve: :sine
      end

      def perc(attackTime = 0.01, releaseTime = 1, level: 1, curve: -4)
        new [0, level, 0], [attackTime, releaseTime], curve: curve
      end

      def linen(attackTime = 0.01, sustainTime = 1, releaseTime = 1, level: 1, curve: :lin)
        new [0, level, level, 0], [attackTime, sustainTime, releaseTime], curve: curve
      end

      def cutoff(releaseTime = 0.1, level: 1, curve: :lin)
        new [level, 0], [releaseTime], curve: curve, release_node: 0
      end

      def dadsr(delayTime = 0.1, attackTime = 0.01, decayTime = 0.3, sustainLevel = 0.5, releaseTime = 1, peakLevel: 1, curve: -4, bias: 0)
        new [0, 0, peakLevel, peakLevel * sustainLevel, 0].collect{ |e| e + bias }, [delayTime, attackTime, decayTime, releaseTime], curve: curve, release_node: 3
      end

      def adsr(attackTime = 0.01, decayTime = 0.3, sustainLevel = 0.5, releaseTime = 1, peakLevel: 1, curve: -4, bias: 0)
        new [0, peakLevel, peakLevel * sustainLevel, 0].collect{ |e| e + bias }, [attackTime, decayTime, releaseTime], curve: curve, release_node: 2
      end

      def asr(attackTime = 0.01, sustainLevel = 1, releaseTime = 1, curve: -4)
        new [0, sustainLevel, 0], [attackTime, releaseTime], curve: curve, release_node: 1
      end
    end
