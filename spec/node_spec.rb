...
describe Node do
  ...
  describe 'Server interaction' do
    ...
    it 'should send free' do
      ...
      wait
      unwind(@server.log).should =~ %r{\[ "/n_free", #{ @node.id } \]}
    end

    it 'should send run' do
      @node.run
      wait
      unwind(@server.log).should =~ %r{\[ "/n_run", #{ @node.id }, 1 \]}
      @node.run false
      wait
      unwind(@server.log).should =~ %r{\[ "/n_run", #{ @node.id }, 0 \]}
    end
    ...
    describe 'map' do
      it 'should just accept instances of Bus' do
        ...
        b1.should_receive(:is_a?).and_return(false)
      ...
      it 'should send map' do
        ...
        b1.should_receive(:is_a?).and_return(true)

        b2 = double 'Bus', index: 2, channels: 2, rate: :audio
        b2.should_receive(:is_a?).and_return(true)

        @node.map(freq1: b1, freq2: b2).should be_instance_of(Node)

        wait
        unwind(@server.log).should =~ %r{\[ \"#bundle\", 1, \n\s+\[ \"/n_mapn\", #{ @node.id }, \"freq1\", #{ b1.index }, 1 \],\n\s+\[ \"/n_mapan\", #{ @node.id }, \"freq2\", #{ b2.index }, 2 \]\n\]}
      end
    end
  end
end
