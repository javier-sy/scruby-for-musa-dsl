  ...
  describe 'messaging' do
    ...
    describe 'set' do
      it 'should send set message with one value' do
        @bus.set 101
        wait
        unwind(@server.log).should =~ %r{\[ "/c_set", #{ @bus.index }, 101 \]}
      end

      it 'should accept value list and send set with them' do
        @bus.set 101, 202
        wait
        unwind(@server.log).should =~ %r{\[ "/c_set", #{ @bus.index }, 101, #{ @bus.index + 1}, 202 \]}
      end
    ...
    describe 'set' do
      it 'should send fill just one channel' do
        @bus.fill 101, 1
        wait
        unwind(@server.log).should =~ %r{\[ "/c_fill", #{ @bus.index }, 1, 101 \]}
      end

      it 'should fill all channels' do
        @bus.fill 101
        wait
        unwind(@server.log).should =~ %r{\[ "/c_fill", #{ @bus.index }, 4, 101 \]}
      end
